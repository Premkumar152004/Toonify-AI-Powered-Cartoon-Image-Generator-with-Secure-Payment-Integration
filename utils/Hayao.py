"""
Hayao Miyazaki Style Processor using ONNX
Soft watercolor Ghibli-style animation effect
"""
import cv2
import numpy as np
...
MODEL_PATH = "anime_models/Hayao.onnx"
...
def load_model():
    ...
    if session is None:
        ...
        print(f"✅ Hayao model loaded from {MODEL_PATH}")
    return session
...
def apply_hayao_style(image):
    """
    Apply Hayao Miyazaki anime style to image
    ...
    try:
        ...
        print(f"🔄 Hayao: Processing {original_size[0]}x{original_size[1]} image...")
        output = model.run([output_name], {input_name: input_tensor})[0]
        
        # Postprocess
        result = postprocess(output, original_size)
        
        print(f"✅ Hayao: Style applied successfully")
        return result
        
    except Exception as e:
        print(f"❌ Error applying Hayao style: {e}")
        import traceback
        traceback.print_exc()
        return image